        ...
        if (in_array($image_ext, $allowed_ext)) {
            ...
            if ($image_size <= 2 * 1024 * 1024) {
                ...
                if (move_uploaded_file($image_tmp, $upload_path)) {
                    ...
                    if ($stmt->execute()) {
                        echo "<script>alert('Product berhasil ditambahkan!'); window.location.href='kelengkapan.php';</script>";
                    } else {
                        echo "<script>alert('Gagal Menambahkan Product'); window.location.href='kelengkapan.php';</script>" . $conn->error;
                    }

                    $stmt->close();
                } else {
                    echo "<script>alert('Gagal mengunggah gambar'); window.location.href='kelengkapan.php';</script>";
                }
            } else {
                echo "<script>alert('Ukuran file gambar terlalu besar (maksimal 2MB).'); window.location.href='kelengkapan.php';</script>";
            }
        } else {
            echo "<script>alert('Format file tidak valid. Hanya JPG, JPEG, PNG, atau GIF yang diperbolehkan.'); window.location.href='kelengkapan.php';</script>";
        }
    } else {
        echo "<script>alert('Harap unggah gambar produk.'); window.location.href='kelengkapan.php';</script>";
    }
}
