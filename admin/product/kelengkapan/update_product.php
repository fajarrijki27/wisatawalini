                ...
                if ($image_size <= 2 * 1024 * 1024) {
                    ...
                    if (move_uploaded_file($image_tmp, $upload_path)) {
                        ...
                        if ($updateImageStmt->execute()) {
                            echo "<script>alert('Product berhasil diperbarui!'); window.location.href='kelengkapan.php';</script>";
                        } else {
                            echo "<script>alert('Gagal memperbarui gambar produk.'); window.location.href='kelengkapan.php';</script>";
                        }

                        $updateImageStmt->close();
                    } else {
                        echo "<script>alert('Gagal mengunggah gambar produk.'); window.location.href='kelengkapan.php';</script>";
                    }
                } else {
                    echo "<script>alert('Ukuran gambar terlalu besar, maksimal 2MB.'); window.location.href='kelengkapan.php';</script>";
                }
            } else {
                echo "<script>alert('Format gambar tidak valid. Hanya JPG, JPEG, PNG, dan GIF yang diperbolehkan.'); window.location.href='kelengkapan.php';</script>";
            }
        } else {
            echo "<script>alert('Product berhasil diperbarui!'); window.location.href='kelengkapan.php';</script>";
        }
    } else {
        echo "<script>alert('Gagal memperbarui produk.'); window.location.href='kelengkapan.php';</script>";
    }
