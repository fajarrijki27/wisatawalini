...
$sql = "SELECT * FROM products WHERE kategori_product = 'Penginapan'";
...
<body class="layout-fixed sidebar-expand-lg bg-body-tertiary"> <!--begin::App Wrapper-->
                    ...
                    <ul class="nav sidebar-menu flex-column" data-lte-toggle="treeview" role="menu" data-accordion="false">
                        ...
                        <li class="nav-item"> 
                            ...
                            <ul class="nav nav-treeview" role="group">
                                ...
                                <li class="nav-item"> 
                                    <a href="penginapan.php" class="nav-link" role="menuitem"> 
                                        <i class="nav-icon bi bi-circle"></i>
                                        <p>Penginapan</p>
                                    </a>
                                </li>
        ...
        <main class="app-main">
            ...
            <div class="app-content-header">
                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            <h3 class="mb-0">Tabel Transaksi Penginapan</h3>
                        </div>
                    </div> <!--end::Row-->
                </div> <!--end::Container-->
                        ...
                        <div class="col-md-12">
                            <div class="card mb-4">
                                <div class="card-header">
                                    <h3 class="card-title">Product Penginapan List</h3>
                                </div> <!-- /.card-header -->
                                        ...
                                        <div class="card-body">
                                            <div class="table-responsive">
                                                <table class="table table-bordered table-hover">
                                                    ...
                                                    <tbody>
                                                    <?php
                                                        $sql = "SELECT p.*, d.discount_value, p.status_penginapan
                                                        FROM products p
                                                        LEFT JOIN discounts d ON p.id_product = d.id_product
                                                        WHERE p.kategori_product = 'Penginapan'";                                                                                               
                                                $result = $conn->query($sql);
                                                ...
                                                if ($result->num_rows > 0) {
                                                    while ($row = $result->fetch_assoc()) {
                                                        ...
                                                        $is_disabled = !empty($row['status_penginapan']); // Menentukan jika produk tidak bisa diproses
                                                        ?>
                                                        <tr 
                                                        data-id-product="<?= $row['id_product']; ?>" 
                                                        data-id-discount="<?= isset($row['discount_value']) ? $row['discount_value'] : 0; ?>" 
                                                        data-status="<?= $row['status_penginapan']; ?>"
                                                        data-nama-product="<?= htmlspecialchars($row['nama_product']); ?>" 
                                                        data-username="<?= $_SESSION['username']; ?>"
                                                        >
                                                            <td><?= htmlspecialchars($row['nama_product']); ?></td>
                                                            <td>
                                                                <input type="number" class="form-control qty-input" value="1" data-harga="<?= $harga; ?>" data-diskon="<?= $diskon; ?>" <?= ($row['status_penginapan'] == 'terisi') ? 'disabled' : ''; ?> />
                                                            </td>
                                                            <td><?= number_format($harga, 2); ?></td>
                                                            <td>
                                                                <select class="form-select diskon-select" <?= ($row['status_penginapan'] == 'terisi') ? 'disabled' : ''; ?>>
                                                            ...
                                                            <td>
                                                                <?php if ($row['status_penginapan'] == 'terisi'): ?>
                                                                    <!-- Tombol untuk mengubah status menjadi kosong -->
                                                                    <button class="btn btn-warning btn-active" onclick="setEmpty(this)" data-id-product="<?= $row['id_product']; ?>">Active</button>
                                                                <?php else: ?>
    ...
    <script>
        ...
        var idProduct = button.getAttribute('data-id-product');
        
        // Memastikan pengguna ingin mengubah status penginapan
        if (confirm('Apakah Anda yakin ingin mengubah penginapan ini menjadi kosong?')) {
            // Mengirim permintaan untuk mengubah status penginapan menjadi kosong
            var url = 'update_status.php?id_product=' + idProduct + '&status=kosong';
            window.location.href = url;  // Mengarahkan ke halaman untuk memperbarui status
        }
    }
